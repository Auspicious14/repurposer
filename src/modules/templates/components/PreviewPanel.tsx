  ...
  return (
    <div className="card p-6 rounded-xl shadow-lg">
      <div className="flex items-center justify-between mb-6">
        ...
        {previewContent && showSaveOutput && (
          <button
            onClick={handleSaveOutput}
            className="px-4 py-2 bg-[var(--secondary)] text-white rounded-lg text-sm font-medium hover:opacity-90 transition-all duration-200"
          >
            Save Output
          </button>
        )}
...
const EmptyState: React.FC = () => (
  <div className="flex items-center justify-center h-full text-center">
    <div className="text-[var(--text-secondary)]">
      ...
      <p className="text-sm">Fill in the template content and select a tone to see the live preview</p>
    </div>
  </div>
);
...
const SuccessState: React.FC = () => (
  <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
    <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
      ...
      <span className="text-sm font-medium">Preview generated successfully!</span>
    </div>
  </div>
);
